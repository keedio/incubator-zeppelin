...
function start() {
  ...
  nohup nice -n $ZEPPELIN_NICENESS $ZEPPELIN_RUNNER -agentlib:jdwp=transport=dt_socket,server=y,address=8888,suspend=n $JAVA_OPTS -cp $CLASSPATH $ZEPPELIN_MAIN >> "${ZEPPELIN_OUTFILE}" 2>&1 < /dev/null &
  pid=$!
  if [[ -z "${pid}" ]]; then
